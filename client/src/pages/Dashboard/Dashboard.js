...
import { Container, Button, Columns, Column, Image } from 'bloomer';
...
class Dashboard extends Component {
  ...
  componentDidMount =() => {
    axios.get("/api/events/").then(events => {
      ...
      this.getEvents(events, this.state.user, this.event.organizer, )
    }) 
  }
  handleLogout = () => {
  ...
  getEvents = (array, ifThis, equalsThat, stateParameter) => {
    var newArray = [];
    array.map(event => {
      if(ifThis = equalsThat) {
        newArray.push(event)
      }
    })
  ...
  getEvents = () => {
    axios.get("/api/events/").then(events=> {
      var eventsArray =[];
      events.map(event=>{
        if(this.state.user.intrests.includes(event.category)) {
